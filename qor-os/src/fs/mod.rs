...
static mut GLOBAL_FILESYSTEM_INTERFACE: Option<
    alloc::sync::Arc<libutils::sync::Mutex<FilesystemInterface>>,
> = None;
...
pub fn init_global_filesystem(_: libutils::sync::InitThreadMarker) {
    ...
    unsafe {
        GLOBAL_FILESYSTEM_INTERFACE.insert(alloc::sync::Arc::new(libutils::sync::Mutex::new(
            FilesystemInterface::new(),
        )))
    };
}
...
pub fn global_filesystem_arc() -> alloc::sync::Arc<libutils::sync::Mutex<FilesystemInterface>> {
    unsafe { &GLOBAL_FILESYSTEM_INTERFACE }
        .clone()
        .expect("Global Filesystem Not Initialized")
}
