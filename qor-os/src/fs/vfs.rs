...
use crate::{fs::FileSystemError, kdebugln, types::DeviceIdentifier};

use super::{DirectoryEntry, FileMode, FileStat, FileSystem, FilesystemResult, InodePointer};
...
impl FilesystemInterface {
    ...
    ) -> FilesystemResult<DeviceIdentifier> {
    ...
    pub fn fs_from_device_error(
        &mut self,
        id: DeviceIdentifier,
    ) -> FilesystemResult<&mut Box<dyn FileSystem>> {
        self.fs_from_device(id)
            .ok_or(FileSystemError::UnableToFindDevice(id))
    }
    ...
    pub fn root_fs_error(&mut self) -> FilesystemResult<&mut Box<dyn FileSystem>> {
        self.root
            .ok_or(FileSystemError::MissingRootMount)
            .map(|id| self.fs_from_device_error(id))?
    }
    ...
    pub async fn index_from(
        ...
        match self.dir_entries(inode).await {
            Ok(entries) => {
                self.index
                    .insert(OwnedPath::new(path.as_str().to_string() + "/"), inode);

                for entry in entries {
                    self.index_from(
                        OwnedPath::new(format!("{}/{}", path, entry.name)),
                        entry.index,
                    )
                    .await?;
                }
    ...
    pub fn inode_to_path(&mut self, inode: InodePointer) -> FilesystemResult<PathBuffer> {
        self.indexed
            .get(&inode)
            .ok_or(FileSystemError::BadInode(inode))
    }
}
...
impl FileSystem for FilesystemInterface {
    ...
    async fn set_mount_id(
        &mut self,
        _mount_id: DeviceIdentifier,
        _interface: &mut FilesystemInterface,
    ) -> FilesystemResult<()> {
    ...
    async fn stat_inode(&mut self, inode: InodePointer) -> FilesystemResult<FileStat> {
        self.fs_from_device_error(inode.device_id)?
            .stat_inode(inode)
            .await
    }
