...
pub fn setup_block_device(ptr: *mut u32) -> bool {
	unsafe {
		...
		ptr.add(MmioOffsets::GuestPageSize.scale32()).write_volatile(4096 as u32);
		...
		ptr.add(MmioOffsets::QueuePfn.scale32()).write_volatile(queue_pfn / 4096 as u32);
...
pub fn block_op(dev: usize, buffer: *mut u8, size: u32, offset: u64, write: bool) {
	unsafe {
		if let Some(bdev) = BLOCK_DEVICES[dev - 1].as_mut() {
			...
			let blk_request = Box::leak(Box::new([0u8; size_of::<Request>()])).as_mut_ptr() as *mut Request;
