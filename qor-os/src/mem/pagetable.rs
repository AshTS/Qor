...
impl TableEntry
{
    ...
    {
        (((self.full_ppn() << 12) as usize) as *mut PageTable).as_mut().unwrap()
    }
}
...
impl RWXFlags
{
    ...
    pub fn read(&self) -> bool
    {
        matches!(self, RWXFlags::ReadExecute | RWXFlags::ReadOnly | RWXFlags::ReadWrite | RWXFlags::ReadWriteExecute )
    }
    ...
    pub fn execute(&self) -> bool
    {
        matches!(self, RWXFlags::Execute | RWXFlags::ReadExecute | RWXFlags::ReadWriteExecute )
    }
}
...
impl PageTable
{
    ...
    pub fn translate_to_entry(&self, virt: VirtualAddress) -> Option<(TableEntry, usize)>
    {
        ...
        {
            let result = (entry_level2.ppn(MemoryPageLevel::Level2) << 30) | virt.page_offset_level(MemoryPageLevel::Level2);
        ...
        {
            let result = (entry_level1.ppn(MemoryPageLevel::Level2) << 30) | (entry_level1.ppn(MemoryPageLevel::Level1) << 21) | virt.page_offset_level(MemoryPageLevel::Level1);
        ...
        {
            let result = (entry_level0.ppn(MemoryPageLevel::Level2) << 30) | (entry_level0.ppn(MemoryPageLevel::Level1) << 21) | (entry_level0.ppn(MemoryPageLevel::Level0) << 12) | virt.page_offset_level(MemoryPageLevel::Level0);
    ...
    pub fn map(&mut self, virt: VirtualAddress, phys: usize, access_flags: RWXFlags, flags: UGFlags, level: MemoryPageLevel)
    {
        ...
        for entry in &mut self.0
        {
            ...
            if entry.valid()
            {
                ...
                if !entry.is_leaf()
                {
                    ...
                    unsafe
                    {
                        crate::mem::PAGE_ALLOCATOR.free_pages_unchecked(
                            no_interrupts, 
                            next_level as *mut PageTable as *mut crate::mem::Page, 1)
