...
impl GlobalPageAllocator
{
    ...
    pub fn initialize(&self, init_thread: InitThreadMarker, _no_interrupts: NoInterruptMarker) -> Result<(), GlobalPageAllocatorError>
    {
        ...
        kdebugln!(init_thread, KernelPageTable, "Initializing Page Heap with {} pages ({} KiB)", heap_page_count, heap_page_count * PAGE_SIZE);
        ...
        *self.data.spin_lock() = Some(
            GlobalPageAllocatorData
            {
                bitmap: core::ptr::Unique::new(bitmap_address).expect("Qor Kernel Requires a Heap"),
                count: final_heap_page_count,
                start_addr: core::ptr::NonNull::new(page_heap_address).expect("Qor Kernel Requires Non Null Heap")
            }
        );
    ...
    pub fn allocate_pages_raw(&self, _no_interrupts: NoInterruptMarker, page_count: usize) -> Result<(*mut Page, usize), GlobalPageAllocatorError>
    {
    ...
    pub fn allocate_static_pages_data<T: Copy>(&self, no_interrupts: NoInterruptMarker, page_count: usize, fill: T) -> Result<&'static mut[T], GlobalPageAllocatorError>
    {
    ...
    pub fn allocate_static_pages(&self, no_interrupts: NoInterruptMarker, page_count: usize) -> Result<&'static mut[Page], GlobalPageAllocatorError>
    {
        let (pointer, length) = self.allocate_pages_raw(no_interrupts, page_count)?;
    ...
    pub fn allocate_static<T>(&self, no_interrupts: NoInterruptMarker, data: T) -> Result<&'static mut T, GlobalPageAllocatorError>
    {
    ...
    pub fn allocate_pages<T>(&self, no_interrupts: NoInterruptMarker, page_count: usize, data: T) -> Result<KernelPageBox<T>, GlobalPageAllocatorError>
    {
    ...
    pub fn allocate<T>(&self, no_interrupts: NoInterruptMarker, data: T) -> Result<KernelPageBox<T>, GlobalPageAllocatorError>
    {
        self.allocate_pages(no_interrupts, (core::mem::size_of::<T>() + PAGE_SIZE - 1) / PAGE_SIZE, data)
    }
    ...
    pub unsafe fn free_pages_unchecked(&self, _no_interrupts: NoInterruptMarker, start_pointer: *mut Page, page_count: usize) -> Result<(), GlobalPageAllocatorError>
    {
        ...
        if let Some(data) = &mut *self.data.spin_lock()
        {
            ...
            for add in 0..page_count
            {
                ...
                {
                    return Err(GlobalPageAllocatorError::MemoryNotAllocated(start_pointer, page_count));
                }
            }
...
impl<T> KernelPageBox<T>
{
    ...
    {
        libutils::sync::no_interrupts(|no_interrupts| crate::mem::PAGE_ALLOCATOR.allocate(no_interrupts, data))
    }
