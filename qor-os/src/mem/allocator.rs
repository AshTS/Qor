...
impl AllocationChunk
{
    ...
    pub fn split(&mut self, size: usize, other: &mut Option<AllocationChunk>)
    {
        ...
        if size < self.size
        {
            ...
            self.next = Some(core::ptr::Unique::new(other as *mut Option<AllocationChunk>).unwrap());
        }
        else
        {
            self.free = false;
...
impl core::fmt::Display for AllocationChunk
{
    ...
    {
        write!(f, "<{:#x} byte{} {} at {:#x}>", self.size, if self.size > 1 { "s" } else { "" }, if self.free { "free" } else { "allocated" }, self.ptr)
    }
}
...
impl Allocator
{
    ...
    pub unsafe fn new(chunks: &'static mut [Option<AllocationChunk>], pointer: usize, size: usize) -> Self
    {
        Self
        {
            chunks,
    ...
    {
        let chunk_index = self.find_free_chunk().expect("Unable to find another free chunk");
...
impl GlobalAllocator
{
    ...
    pub fn initialize(&self, _init_thread: InitThreadMarker, no_interrupts: NoInterruptMarker)
    {
        ...
        let pages = 512;
        let entries = crate::mem::PAGE_ALLOCATOR.allocate_static(no_interrupts, [None; 4096]).expect("Unable to allocate allocation entries");

        let pointer = crate::mem::PAGE_ALLOCATOR.allocate_static_pages(no_interrupts, pages).expect("Unable to allocate space for heap").as_ptr();

        unsafe { self.allocator.get().as_mut().unwrap().insert(Allocator::new(entries, pointer as usize, crate::mem::PAGE_SIZE * pages)) };
    }
}
