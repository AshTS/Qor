...
pub fn setup_block_device(ptr: *mut u32) -> bool
{
    ...
    unsafe { super::mmio::write_offset::<u32>(ptr as usize, MmioOffsets::Status as usize, 0) };

    let mut status_bits = StatusField::Acknowledge.val32();
    // 2. Set ACKNOWLEDGE status bit
    unsafe { super::mmio::write_offset::<u32>(ptr as usize, MmioOffsets::Status as usize, status_bits) };
    ...
    let host_features = unsafe { super::mmio::read_offset::<u32>(ptr as usize, MmioOffsets::HostFeatures as usize)};
    let guest_features = host_features & !(1 << VIRTIO_BLK_F_RO);
    let ro = host_features & (1 << VIRTIO_BLK_F_RO) != 0;
    unsafe { super::mmio::write_offset::<u32>(ptr as usize, MmioOffsets::GuestFeatures as usize, guest_features)};
    // 5. Set the FEATURES_OK status bit
    status_bits |= StatusField::FeaturesOk.val32();
    unsafe { super::mmio::write_offset::<u32>(ptr as usize, MmioOffsets::Status as usize, status_bits)};
    // 6. Re-read status to ensure FEATURES_OK is still set.
    // Otherwise, it doesn't support our features.
    let status_ok = unsafe { super::mmio::read_offset::<u32>(ptr as usize, MmioOffsets::Status as usize)};
    ...
    if false == StatusField::features_ok(status_ok)
    {
        ...
        unsafe { super::mmio::write_offset::<u32>(ptr as usize, MmioOffsets::Status as usize, StatusField::Failed as u32)};
        return false;
    }
    ...
    // a certain size.
    let qnmax = unsafe { super::mmio::read_offset::<u32>(ptr as usize, MmioOffsets::QueueNumMax as usize)};
        unsafe { super::mmio::write_offset::<u32>(ptr as usize, MmioOffsets::QueueNum as usize, VIRTIO_RING_SIZE as u32)};
    ...
    unsafe 
    {
        ...
        super::mmio::write_offset::<u32>(ptr as usize, MmioOffsets::QueuePfn as usize, queue_pfn / 4096 as u32);
        ...
        super::mmio::write_offset::<u32>(ptr as usize, MmioOffsets::Status as usize, status_bits);
    }
...
pub fn block_op(dev: usize, buffer: *mut u8, size: u32, offset: u64, write: bool) -> Option<*mut Request>
{
    if let Some(bdev) = unsafe { BLOCK_DEVICES[dev - 1].as_mut() }
    {
        ...
        unsafe
        {
            ...
            super::mmio::write_offset::<u32>(bdev.dev as usize, MmioOffsets::QueueNotify as usize, 0);
        }
...
{
    if let Some(bdev) = unsafe { BLOCK_DEVICES[idx].as_mut() }
    {
